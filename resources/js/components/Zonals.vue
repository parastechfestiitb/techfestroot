<template>
    <div class="controller" :style="{height:controllerHeight + 'px'}" @keyup.left="moveBlock(-1)" @keyup.right="moveBlock(1)">
        ...
        <main class="d-flex flex-row d-none zi1">
            <div class="navigation-compi d-flex flex-row active">
                <div class="content-nav-compi d-flex align-items-center" v-if="elements!==null">
                    <div class="content">
                        ...
                        <div v-for="(elem,key) in elems().slice(1,elems().length-1)" v-if="elem.zonal!==1" class="nav-compi-item pointer" @click="$root.routeChange('/competitions')">
                            {{elem.name}}
                        </div>
                        <br>
                        <div class="compi-title nav-compi-item pointer" @click="changeInfo(0)">TechnoRion</div>
                        ...
                        <div class="nav-compi-item pointer" @click="$root.routeChange('/ideate')">Transforming The World</div>
                    </div>
                </div>
            ...
            <div class="block" v-for="(val,key) in elemsZonals()" :style="{ transform : 'translateX(' + position(key) +'vw)', width: wid(key) + 'vw'}" :id="'block-'+key" :class="[key===elementLength?'fixed-block pen':'',key===frame?'active-block':'passive-block','compi-'+val.name]" @click="changeInfo(key)">
                <div v-if="key===0" class="relative">
                    ...
                    <div class="short-description-first" :style="{opacity: opacityPrizeMoney1(key), left: leftName(key) + 'vw'}">
                        Providing a chance for the innovative minds across the nation to showcase their technical skills has been the motto of Techfest Competitions.
                        We don’t want distance to play a role in hindering innovative minds from participating in Techfest Competitions.
                        <br><br>
                        <table class="zonal-table">
                            ...
                            <tr>
                                <td>Kolkata</td>
                                <td>23 Sept.</td>
                                <td>University of Engineering &amp; Management, Kolkata</td>
                            </tr>
                            <tr>
                                <td>Bhopal</td>
                                <td>23 Sept.</td>
                                <td>Oriental Group of Institutes, Bhopal</td>
                            </tr>
                            <tr>
                                <td>Mumbai</td>
                                <td>30 Sept.</td>
                                <td>IIT Bombay, Mumbai</td>
                            </tr>
                            <tr>
                                <td>Jaipur</td>
                                <td>30 Sept.</td>
                            ...
                            <tr>
                                <td>Bangalore</td>
                                <td>30 Sept.</td>
                                <td>Universite Visvesvaraya College of Engineering</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="name-first play-font" :style="{fontSize: fontName(key) + 'vw',left: (leftName(key)+4) + 'vw'}">TechnoRion</div>
                    <button class="explore-button explore-more play-font" :style="{left: leftName(key) + 'vw'}" @click="changeInfo(1)">Explore Zonals →</button>
                    <button class="explore-button explore-zonal play-font" :style="{left: (leftName(key) - 4.906) + 'vw'}" @click="$root.routeChange('/competitions')"competition>Explore All Competitions</button>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            elemsZonals: function(){
                if(this.elements !== null)
                return this.elements.filter((e)=>{
                    return e.zonal===1||e.name==='Competitions';
                });
            },
            numberWithCommas: (x) => {
            ...
            changeInfo: function(e){
                ...
                if(e===0) return false;
                console.log(this.elemsZonals()[e].url);
                this.$root.routeChange(this.elemsZonals()[e].url);
                return false;
            },
            goToFirst: function(){
                $("html").stop().animate({
