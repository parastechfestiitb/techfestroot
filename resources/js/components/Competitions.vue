<template>
    <div class="controller" :style="{height:controllerHeight + 'px'}" @keyup.left="moveBlock(-1)" @keyup.right="moveBlock(1)">
        <div class="background-events bgp zi0">
            ...
            <img v-for="v in portalImg" :src="s3.replace(':img','portal-parts.png')" :alt="'portal part '+v" class="part" :class="'part-'+v" :style="{right: mex*partSen[v]*(op*1.2<100?(1-op*1.2/100):0) + 'vw',bottom: met*partSen[v]*(op*1.2<100?(1-op*1.2/100):0) + 'vw'}">
            <img :src="s3.replace(':img','event-glow.png')" alt="back-to-start" class="event-glow" :class="op*1.2>100?'active':'pen'">
        </div>
        <main class="d-flex flex-row d-none zi1">
            <div class="navigation-compi d-flex flex-row active">
                <div class="content-nav-compi d-flex align-items-center" v-if="elements!==null">
                    <div class="content">
                        <div class="compi-title nav-compi-item pointer" @click="changeInfo(0)">Competitions</div>
                        <div v-for="(elem,key) in elems().slice(1,elems().length-1)" v-if="elem.zonal!==1" class="nav-compi-item pointer" @click="changeInfo(key+1)">
                            {{elem.name}}
                        </div>
                        <div class="compi-title nav-compi-item pointer" @click="$root.routeChange('/technorion')">Wildcard</div>
                        <div v-for="(elem,key) in elems().slice(1,elems().length-1)" v-if="elem.zonal===1" class="nav-compi-item pointer" @click="changeInfo(key+1)">
                            {{elem.name}}
                        </div>
                        <div class="compi-title nav-compi-item pointer" @click="$root.routeChange('/ideate')">Ideate</div>
                        <div class="nav-compi-item pointer" @click="$root.routeChange('/ideate')">Vision 2030</div>
                        <div class="nav-compi-item pointer" @click="$root.routeChange('/ideate')">Reforming the Past</div>
                        <div class="nav-compi-item pointer" @click="$root.routeChange('/ideate')">BEL Innovate to illuminate</div>
                    </div>
                </div>
            ...
            <div class="block" v-for="(val,key) in elems()" :style="{ transform : 'translateX(' + position(key) +'vw)', width: wid(key) + 'vw'}" :id="'block-'+key" :class="[key===elementLength?'fixed-block pen':'',key===frame?'active-block':'passive-block','compi-'+val.name]" @click="changeInfo(key)">
                <div v-if="key===0" class="relative">
                    ...
                    <!--<button class="explore-button explore-more play-font" :style="{left: leftName(key) + 'vw'}" @click="changeInfo(1)">Explore All Competitions →</button>-->
                    <div class="img-block zonal-container" :style="{height: imgBlockHeight(key) +'vh'}" alt="Competition">
                        <div class="zonal-title">TechnoRion</div>
                        <div class="zonal-description">Providing a chance for the innovative minds across the nation to showcase their technical skills has been the motto of Techfest Competitions. We don’t want distance to play a role in hindering innovative minds from participating in Techfest Competitions.</div>
                        <div class="zonal-dates" :style="{left: (leftName(key)-4.8) + 'vw'}">
                            After a remarkable turn up at the technorion, Techfest Zonals held in 5 major cities of India,
                            <br>
                            Registrations for wildcard round for TechnoRion are open now.
                        </div>
                        <!--<div class="zonal-region">-->
                            <!--<button class="explore-button explore-zonal play-font" :style="{left: (leftName(key) - 4.906) + 'vw'}" @click="$root.routeChange('/zonals')">Explore TechnoRion</button>-->
                        <!--</div>-->
                    </div>
                </div>
                <div v-if="key!==elementLength && key!==0" class="relative">
                    ...
                    <div class="compi zonal-event" v-if="val.zonal===1" :style="{opacity:opacityPrizeMoney1(key)}">Wildcard</div>
            ...
            <div class="back-to-start-text" v-if="op*1.2>100" @click="goToFirst">BACK TO<br>START</div>
        </main>
...
<script>
    ...
    export default {
        ...
        created: function () {
            axios.post(_routes.competitionsPost)
                .then((response) => {
                    ...
                    if (navigator.userAgent.indexOf('Mac OS') !== -1) {
                        window.addEventListener('mousewheel',this.macHandleScroll);
                    } else {
                        window.addEventListener('scroll', this.handleScroll);
                    }
