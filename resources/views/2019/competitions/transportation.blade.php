...
<head>
    ...
    <title>Unravelling Transportation | Techfest, IIT Bombay</title>
    ...
    <style>
        ...
        @media (min-width:992px) {
            ...
            .mobile_hide{
                ...
                background-image: url(/2019/compi/images/transportation.jpg) ;
...
<div id="colorlib-page"  >
    ...
    <aside id="colorlib-aside" role="complementary" class="border js-fullheight" >
        ...
        <a href="https://www.jlt.com/"><img  src="/2019/compi/images/transportation_logo.png" alt="" style="width: 100%; padding-top: 10%;"></a>
        <nav id="colorlib-main-menu" role="navigation" style="margin-top: 10vh; font-size: medium; ">
            <ul>
                ...
                <li ><a onclick="myFunction2_1()">Timeline</a></li>
                ...
                <div class="row ">
                    ...
                    <div class="col-md-8 animate-box" data-animate-effect="fadeInLeft" style="margin-right:-1%; padding-left:2.5% ;padding-right: -50%">
                        <div class="about-desc" style="margin-right: 0%; padding: 0px" >
                            ...
                            <h2 style="color: white; font-weight: bold">Unravelling Transportation</h2>
                            <hr style="color: white; border: 1px solid white">
                            <h3 style="color: white">Prize Money: INR 1,25,000/- </h3>

                            <p style=" font-size: 15px; width:90%; font-family: Lato;color: white ">
                                Develop innovative technological solutions for better management of transport and traffic systems.
                            </p>

                            @if(!empty($user_row->email))
                                <?php

                                $team =  DB::table('tf_reg')->where(['transportation_team' => $user_row->transportation_team])->get();
                                $team_leader_row = DB::table('tf_reg')->where(['email' => $team[0]->transportation_team])->first();

                                if($user_row->transportation_team == $user_row->email ){
                                    $team_leader = $user_row->name;
                                }
                                ?>
                            @endif

                            <a href="/2019/compi/ps/Unravelling Transportation.pdf" target="_blank" class="btn btn-primary btn-learn">Problem Statement</a>
...
{{--                                    <a href="#" class="btn btn-primary btn-learn" >Registrations</a>--}}
                                ...
                                @if($user_row->transportation == 1 && empty($user_row->transportation_team))
                                    <div class="dropdown ">
                                        ...
                                        <div class="dropdown-content">
                                            <a href="/competitions/transportation/createteam">Create Team</a>
                                            <a href="/competitions/transportation/jointeam">Join Team</a>
                                        </div>
                                    </div>
                                @endif

                                @if(!empty($user_row->transportation_team))
                                    @if($user_row->transportation_team != $user_row->email )
                                        <a href="/competitions/transportation/leaveteam" class="btn btn-primary btn-learn" >Leave current Team</a>
                                    @endif
                                    @if($user_row->transportation_team == $user_row->email )
                                        <a href="/competitions/transportation/dissolveteam" class="btn btn-primary btn-learn" >Dissolve the team</a>
                                        <div class="dropdown ">
                                            <button class=" btn btn-primary btn-learn">Remove a Member</button>
                                            <div class="dropdown-content">
                                                @if(!empty($team[0]->name) and ($team[0]->email != $team_leader_row->email) )
                                                    <a href="/competitions/transportation/remove/{{$team[0]->id}}">{{$team[0]->name}}</a>
                                                @endif
                                                @if(!empty($team[1]->name) and ($team[1]->email != $team_leader_row->email) )
                                                    <a href="/competitions/transportation/remove/{{$team[1]->id}}">{{$team[1]->name}}</a>
                                                @endif
                                                @if(!empty($team[2]->name) and ($team[2]->email != $team_leader_row->email) )
                                                    <a href="/competitions/transportation/remove/{{$team[2]->id}}">{{$team[2]->name}}</a>
                                                @endif
                                                @if(!empty($team[3]->name) and ($team[3]->email != $team_leader_row->email) )
                                                    <a href="/competitions/transportation/remove/{{$team[2]->id}}">{{$team[3]->name}}</a>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @if(!empty($user_row))
                                @if($user_row->transportation > 0 && empty($user_row->transportation_team))
                                    <div class="alert alert-success">
                                        <p><strong>Hi {{$user_row->name}}, you are successfully registered for Transportation with <b>{{$user_row->email}}</b> as your Registered email id , now you must create/join a team.</strong></p>
                                    </div>
                                @endif
                                @if($user_row->transportation > 0 && !empty($user_row->transportation_team))
                                    <div class="" style="    border: solid 2px white;border-radius: 6px;padding: 2%;width: 95%">
                                        <p style="color: white">
                                            ...
                                            Team ID :UT- {{$team_leader_row->transportation_team_id}}
        ...
        <div id="text_change" class="col-md-12 animate-box scroll_inside" data-animate-effect="fadeInLeft" style="
            ...
            <p id="text_change" style="margin-bottom: 0px; color: white">
                <br>
                Participants are invited to come up with innovative technological solutions for better management of transport and traffic systems. Some of the themes which you can work upon are:
                <br>
                <br>• Optimizing traffic light management
                <br>• Fuel efficient, affordable and faster means of transport
                <br>• Public transport inadequacy
                <br>• Connectivity to rural areas
                <br>• Worn out and obsolete Assets
                <br>• Railroads and Subway Systems
                <br>• Strategic Road Network
                <br>• Road Safety
                <br><br>
                Note: The above themes (circular bulleted points) are just guidelines to help you. You are free to think like an unconventional thinker and come up with great innovations. Do not limit yourselves to these themes.
...
<script>
    function myFunction1() {
        document.getElementById("text_change").innerHTML = "<br><u>REPORT FORMAT</u>\n <br>" +
            "\n" +
            "<br>1. Title\n" +
            "\n" +
            "<br><br>2. Abstract\n" +
            ...
            "\n" +
            "<br><br>The project report should be emailed to transportation@techfest.org with the subject ‘Ideate: ‘’Unravelling Transportation’’ Project Report: Team Id’ (for eg. Ideate: ‘’Unravelling Transportation’’ Project Report: EM1234).\n" +
            "All the submissions should be emailed to transportation@techfest.org. Teams must follow the following details for the submission:\n" +
            "<br>The abstract must be submitted in pdf format only\n" +
            " <br><br><u>PROTOTYPE AND FINAL PRESENTATION</u>\n" +
            "\n" +
            "<br><br>After declaration of the finalists, participants are required to improve upon the prototype/ working model of their project. They will also have to make a presentation covering the technical and financial aspects of their product in a detailed manner. This final presentation and the video will have to be submitted at transportation@techfest.org before 10th December 2019. The shortlisted teams can however, continue to work on their prototypes till 2nd January, 2020. The teams will have to bring their prototypes to be judged and showcased at Techfest 2019-20 during 3rd to 5th January, 2020. The working prototype should be as close as possible to the product that the team intends to present to the end user. This would also help in deciding a better estimate of the cost of the prototype, reflecting closely the actual cost of the product\n" +
            "<br><br>";
    }
</script>
...
<script>
    function myFunction2() {
        document.getElementById("text_change").innerHTML = "<br>Q. Who can participate ?\n" +
            ...
            "<br>NO, there will be no mentor panel to clear your doubts.\n <br>" +
            "<br>Q. How to register?<br>Follow this steps for registration :\n" +
            "www.techfest.org > Ideates > Unravelling Transportation > Explore More -> Register > Fill all your details > You will be registered and now you must either Create a team/Join a team<br>" +
            ...
            "Follow this process:- www.techfest.org -> Competitions ->Unravelling Transportation -> Explore More -> Register -> Fill all your details -> Team -> Create a team -> Use the Registered Mail Id of other participants to add them in your team. They must be already registered before adding them in your Team. \n<br>" +
...
<script>
    function myFunction2_1() {
        document.getElementById("text_change").innerHTML = "<br>" +
            ...
            "            <br>Presentation Stage: 3rd January 2020\n" +
...
<script>
    function myFunction3() {
        ...
        document.getElementById("text_change").innerHTML = "<br> " +
            ...
            //     "<br>Participants must send their signed Declaration Form to be considered eligible for participating in the ideate-Transportation. Otherwise they will be disqualified.\n" +
            ...
            "<br>3. A team can register at any point of time before 9th December 2019 and can submit final abstract and video (as mentioned in the structure).\n" +
            ...
            "\n" +
            "<br><br>Transportation abstracts will be judged by a panel of experts. Following are the broad guidelines for judging:\n" +
