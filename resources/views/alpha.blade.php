...
<head>
    ...
    <style>*{box-sizing:border-box;cursor: url(https://techfest.org/2018/cursor.cur), auto;font-family: 'Roboto', sans-serif;font-weight: 300;transition: filter 500ms;}  html, body, #app{width: 100%;height: 100%;background: #000;}  .blur{filter: blur(15px);pointer-events: none;}  img {-moz-user-select: none;-webkit-user-select: none;-ms-user-select: none;user-select: none;-webkit-user-drag: none;user-drag: none;}  a{color: white !important;;text-decoration: underline !important;}</style>
    ...
    <div class="navigation d-flex flex-row-reverse">
        ...
        <div class="block d-flex flex-column">
            ...
            @if(session()->has('participant') && session()->has('admin'))
                <div @click="routeChange('/payment')" class="initiatives nav-button">Payment</div>
            @endif
        </div>
    </div>
        ...
        <div class="search-container">
            ...
            <div class="d-flex justify-content-around search-list play">
                <div class="column">
                    ...
                    <div class="group">
                        <div class="hd1 pointer" @click="routeChange('/robowars/2018')">Robowars</div>
                    </div>
                    {{--<div class="group">--}}
                            ...
                            @if($participant->events()->count()>0)
                                @foreach($participant->events()->get() as $event)
                                    <div class="event">
                                        ...
                                        @if(session()->has('participant') && session()->has('admin') && $event->tickets()->count()!==0)
                                            @if($participant->team($event)->get()[0]->ticket_id===null )
                                                <div class="payment-button button pay-payment global-button" @click="$root.routeChange('/Payment')">
                                                    PAY
                                                </div>
                                            @else
...
<script>
    var giftofspeed = document.createElement('link');giftofspeed.rel = 'stylesheet';giftofspeed.href = '{{asset('css/alpha.css')}}';giftofspeed.type = 'text/css';var godefer = document.getElementsByTagName('link')[0];godefer.parentNode.insertBefore(giftofspeed, godefer);
...
<script src="{{asset('js/alpha.js?v=5.13')}}" defer async></script>{{--Competitions--}}
</body>
</html>
